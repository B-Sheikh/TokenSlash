...
def fetch_lmsys_and_hf_datasets(max_samples=1500):
    """
    ...
    """
    print(f"Fetching public LLM conversation datasets (target: {max_samples} prompts)...")
            ...
            while len(conversations) < max_samples:
                ...
                for line in lines:
                    ...
                    try:
                        ...
                        continue
        print(f"Successfully retrieved {len(conversations)} raw conversation records.")
    except Exception as e:
        print(f"Notice: Streaming fallback triggered ({e}). Loading cached benchmark dataset...")
        conversations = []
...
def get_official_pricing_data():
    """
    Official pricing tables sourced from OpenAI, Anthropic, and Google AI.
    Input/Output costs are in USD per 1M tokens.
    """
    return {
        "gpt-4o-mini": {
            "provider": "OpenAI",
...
def get_ai_benchmark_scores():
    """
    AI Model Capability metadata aggregated from:
    - LiveBench
    - Arena Hard
    - MMLU Pro
    - SWE Bench
...
def build_and_save_dataset_package(output_dir):
    ...
    raw_convs = fetch_lmsys_and_hf_datasets(1500)
    ...
    print(f"Dataset package saved to {out_file} (Size: {len(raw_convs)} conversation samples)")
    return out_file
